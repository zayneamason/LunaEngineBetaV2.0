...
class MLXWhisperSTT(STTProvider):
    ...
    async def transcribe(
        ...
        try:
            ...
            # Run transcription in executor (CPU-bound even with MLX)
    ...
    def _transcribe_file(self, file_path: str) -> TranscriptionResult:
        ...
        try:
            ...
            # MLX-Whisper doesn't return confidence scores
            # We use a high confidence since the model is generally accurate
            ...
            logger.error(f"MLX-Whisper file transcription failed: {e}")
            return TranscriptionResult(text="", confidence=0.0, is_final=True)
