...
function KozmoInner({ onBack }) {
  ...
  // Stable callbacks for child components — prevents re-render cascades
  const saveLabState = useCallback((s) => send({ type: 'SAVE_LAB_STATE', state: s }), [send]);
  const saveCodexState = useCallback((s) => send({ type: 'SAVE_CODEX_STATE', state: s }), [send]);
  const saveScriboState = useCallback((s) => send({ type: 'SAVE_SCRIBO_STATE', state: s }), [send]);
  ...
  return (
    <div className="h-full w-full flex flex-col bg-kozmo-bg text-white font-mono overflow-hidden">
      {/* Top Bar */}
      <header className="flex items-center justify-between px-4 h-10 border-b border-kozmo-border flex-shrink-0"
        ...
        <div className="flex items-center gap-3">
          {onBack && (
            <button
              onClick={onBack}
              className="flex items-center gap-1.5 px-2 py-1 rounded text-kozmo-muted hover:text-white transition-colors"
              style={{ fontSize: 9, background: 'rgba(30,30,46,0.4)', border: '1px solid rgba(30,30,46,0.6)' }}
            >
              <span style={{ fontSize: 11 }}>&larr;</span> ECLISSI
            </button>
          )}
        ...
        {!activeProject ? (
          ...
          mode === MODES.SCRIBO
            ...
            ? <KozmoCodex
                ...
                onSaveState={saveCodexState}
              />
            : <KozmoLab
                savedState={state.context.labState}
                onSaveState={saveLabState}
                onNavigateToScript={(docSlug, containerSlug) => {
                  // Switch to SCRIBO with the selected document + expand parent containers
                  const existingExpanded = state.context.scriboState?.expandedNodes || [];
                  const expandedSet = new Set(existingExpanded);
                  if (containerSlug) expandedSet.add(containerSlug);
                  expandedSet.add('root');
                  const newScriboState = {
                    ...state.context.scriboState,
                    selectedScene: docSlug,
                    expandedNodes: [...expandedSet],
                    _navigatedFromLab: true,
                  };
                  // Set scribo state first, then navigate (both handled in lab state now)
                  send({ type: 'SAVE_SCRIBO_STATE', state: newScriboState });
                  send({ type: 'TO_SCRIBO', scriboState: newScriboState });
                }}
              />
        )}
      </main>
...
export default function KozmoApp({ onBack }) {
  return (
    <KozmoProvider>
      <KozmoInner onBack={onBack} />
    </KozmoProvider>
  );
}
