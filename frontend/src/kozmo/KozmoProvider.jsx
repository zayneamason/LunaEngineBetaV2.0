...
export function KozmoProvider({ children }) {
  ...
  // --- Reverse Index / Keyword Tracking ---
  const [reverseIndex, setReverseIndex] = useState(null);
  const [usageByEntity, setUsageByEntity] = useState({});
  const _refetchTimer = useRef(null);
  const _scheduleRefreshRef = useRef(null);
  ...
  const saveDocument = useCallback(async (docSlug, updates) => {
    ...
    try {
      ...
      if (res.ok && _scheduleRefreshRef.current) _scheduleRefreshRef.current();
      return res.ok;
    } catch { return false; }
  }, [activeProject]);
  ...
  // --- Reverse Index / Keyword Tracking ---
  const fetchReverseIndex = useCallback(async () => {
    if (!activeProject) return null;
    try {
      const res = await fetch(`${API_BASE}/projects/${activeProject.slug}/reverse-index`);
      if (!res.ok) return null;
      const data = await res.json();
      setReverseIndex(data);
      setUsageByEntity(data.entity_usage || {});
      return data;
    } catch { return null; }
  }, [activeProject]);

  // Auto-fetch reverse index when project loads
  useEffect(() => {
    if (activeProject) fetchReverseIndex();
  }, [activeProject?.slug, fetchReverseIndex]);

  // Debounced re-fetch after document saves
  const scheduleReverseIndexRefresh = useCallback(() => {
    if (_refetchTimer.current) clearTimeout(_refetchTimer.current);
    _refetchTimer.current = setTimeout(() => fetchReverseIndex(), 2000);
  }, [fetchReverseIndex]);
  _scheduleRefreshRef.current = scheduleReverseIndexRefresh;

  // Merged entity array for AnnotatedText consumption
  const annotatedEntities = useMemo(() =>
    Object.values(entities).map(entity => ({
      id: entity.slug,
      name: entity.name,
      type: entity.type,
      aliases: entity.aliases || entity.data?.aliases || [],
      mention_count: usageByEntity[entity.slug]?.mention_count || 0,
      profile: entity.data?.description || entity.luna_notes || '',
      relationships: entity.relationships || [],
      references: entity.references || {},
    })),
    [entities, usageByEntity]
  );
  ...
  // --- Eden Reference Image Generation ---
  const generateEntityRef = useCallback(async (entityType, entitySlug, opts = {}) => {
    if (!activeProject) return null;
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(
        `${API_BASE}/projects/${activeProject.slug}/entities/${entityType}/${entitySlug}/generate-ref`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      // Refresh entity data to pick up new image in references
      await loadProject(activeProject.slug);
      return data;
    } catch (e) {
      setError(e.message);
  ...
  const value = {
    ...
    annotatedEntities, reverseIndex, usageByEntity, fetchReverseIndex,
    ...
    rebuildGraph, buildPrompt, generateScene, generateEntityRef,
