...
import ScriboOverlay from './ScriboOverlay';
  ...
  return (
    <div>
      ...
      >
        ...
        >
          {hasChildren && (
            <span style={{ color: '#5a5a6e', fontSize: 10, width: 12, textAlign: 'center', flexShrink: 0 }}>
              {isExpanded ? '▾' : '▸'}
            </span>
          )}
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '0 4px' }}>
      {path.map((item, i) => (
        <span key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          {i > 0 && <span style={{ color: '#3a3a4e', fontSize: 11 }}>›</span>}
          <span
            onClick={() => onNavigate(item.id, item)}
            style={{
              color: i === path.length - 1 ? '#e2e8f0' : '#5a5a6e',
              fontSize: 12, cursor: 'pointer',
              fontFamily: "'JetBrains Mono', monospace",
              transition: 'color 0.15s',
            }}
            onMouseEnter={e => e.currentTarget.style.color = '#c084fc'}
            onMouseLeave={e => e.currentTarget.style.color = i === path.length - 1 ? '#e2e8f0' : '#5a5a6e'}
          >
  ...
  if (!isEditing) {
    ...
    return (
      <div style={{
        ...
        borderBottom: '1px solid #282840', flexWrap: 'wrap', alignItems: 'center',
      }}>
        ...
        <span style={{ color: '#5a5a6e', fontSize: 11, fontFamily: "'JetBrains Mono', monospace" }}>
        ...
        <div>
          ...
          <div style={{
            ...
            {characterEntities.length > 0 ? characterEntities.map(char => (
              ...
              <div style={{ color: '#5a5a6e', fontSize: 11, fontStyle: 'italic', padding: 8 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            ...
            {locationEntities.length === 0 && (
              <div style={{ color: '#5a5a6e', fontSize: 10, fontStyle: 'italic', marginTop: 4 }}>
...
/**
 * FountainAnnotatedLine — Renders a line of text with both Fountain formatting
 * (bold, italic, underline) and entity highlighting via AnnotatedText.
 */
function FountainAnnotatedLine({ text, annotatedEntities, onEntityClick }) {
  // Parse Fountain markers into segments: [{type, text}, ...]
  const segments = useMemo(() => {
    const result = [];
    // Regex for Fountain markers: ***bold italic***, **bold**, *italic*, _underline_
    const pattern = /(\*\*\*.*?\*\*\*|\*\*.*?\*\*|\*.*?\*|_.*?_)/g;
    let lastIdx = 0;
    let match;
    while ((match = pattern.exec(text)) !== null) {
      if (match.index > lastIdx) {
        result.push({ type: 'normal', text: text.slice(lastIdx, match.index) });
      }
      const raw = match[0];
      if (raw.startsWith('***') && raw.endsWith('***')) {
        result.push({ type: 'bold-italic', text: raw.slice(3, -3) });
      } else if (raw.startsWith('**') && raw.endsWith('**')) {
        result.push({ type: 'bold', text: raw.slice(2, -2) });
      } else if (raw.startsWith('*') && raw.endsWith('*')) {
        result.push({ type: 'italic', text: raw.slice(1, -1) });
      } else if (raw.startsWith('_') && raw.endsWith('_')) {
        result.push({ type: 'underline', text: raw.slice(1, -1) });
      }
      lastIdx = match.index + raw.length;
    }
    if (lastIdx < text.length) {
      result.push({ type: 'normal', text: text.slice(lastIdx) });
    }
  ...
  return (
    <>
      {segments.map((seg, i) => {
        ...
        if (seg.type === 'bold-italic') return <strong key={i}><em>{inner}</em></strong>;
        if (seg.type === 'bold') return <strong key={i}>{inner}</strong>;
        if (seg.type === 'italic') return <em key={i}>{inner}</em>;
        if (seg.type === 'underline') return <u key={i}>{inner}</u>;
...
function SceneEditor({ content, entities, annotatedEntities, onNavigateToEntity }) {
  ...
  // AnnotatedText click handler (receives entity slug directly)
  // Must be before early return to satisfy Rules of Hooks
  const handleAnnotatedEntityClick = useCallback((entitySlug) => {
    if (onNavigateToEntity && entitySlug) {
      const entity = entities.find(e => e.slug === entitySlug || e.id === entitySlug);
      onNavigateToEntity(entitySlug, entity?.type);
    }
  }, [entities, onNavigateToEntity]);

  if (!content) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#5a5a6e' }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 48, marginBottom: 16, opacity: 0.3 }}>¶</div>
          <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 16 }}>Select a scene to begin writing</div>
          <div style={{ fontSize: 13, marginTop: 8, color: '#3a3a4e' }}>or create a new scene with ⌘N</div>
        </div>
      </div>
    );
  }
  ...
  // FEATURE 3: Navigate to CODEX handler (supports both DOM events and AnnotatedText slugs)
    ...
    if (isAfterCharacter && trimmed && !/^[A-Z][A-Z\s]{2,}$/.test(trimmed)) {
      return (
        <div key={idx} style={{
          ...
          <FountainAnnotatedLine
            text={trimmed}
            annotatedEntities={annotatedEntities}
            onEntityClick={handleAnnotatedEntityClick}
          />
        </div>
      );
    }
          ...
          <div style={{
            ...
            <button
              ...
              style={{
                ...
                color: newNote.trim() && !isSaving ? '#c084fc' : '#5a5a6e',
                fontSize: 11,
                fontFamily: "'Space Grotesk', sans-serif",
                fontWeight: 500,
...
function CodexSidebar({ entities, sceneCharacters }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{
        padding: '12px 12px 8px', borderBottom: '1px solid #282840',
        display: 'flex', gap: 4,
      }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 8 }}>
        {filtered.length === 0 ? (
          <div style={{ color: '#3a3a4e', fontSize: 12, textAlign: 'center', paddingTop: 24 }}>
            No entities in current scope
          </div>
        ) : filtered.map(entity => (
          <div key={entity.id} style={{
            padding: '10px 12px', marginBottom: 4, borderRadius: 6,
            background: 'rgba(26, 26, 38, 0.5)', cursor: 'pointer',
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              ...
              <span style={{
                color: '#5a5a6e', fontSize: 10, fontFamily: "'JetBrains Mono', monospace", marginLeft: 'auto',
              }}>
                {entity.type}
              </span>
            </div>
...
function WordCountBar({ scene, total }) {
  return (
    <div style={{
      ...
      borderTop: '1px solid #282840', background: 'rgba(20, 20, 30, 0.4)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)',
      ...
      <span style={{ marginLeft: 'auto', color: '#3a3a4e' }}>⌘K commands · ⌘/ agents · ⌘B codex</span>
    </div>
  );
}
...
function CreateStoryItemForm({ mode, tree, onClose, onCreateScene, onCreateContainer }) {
  ...
  return (
    <div style={{
      position: 'absolute', inset: 0, background: 'rgba(16,16,26,0.9)',
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 8, color: '#4a4a62', letterSpacing: 2, marginBottom: 6 }}>TITLE</div>
          <input
            value={title}
        ...
        {mode === 'scene' ? (
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 8, color: '#4a4a62', letterSpacing: 2, marginBottom: 6 }}>CONTAINER</div>
            <select
              value={container}
          ...
          <>
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 8, color: '#4a4a62', letterSpacing: 2, marginBottom: 6 }}>LEVEL</div>
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                {['act', 'chapter', 'sequence', 'section'].map(l => (
                  <button key={l} onClick={() => setLevel(l)} style={{
                    padding: '4px 10px', fontSize: 9, borderRadius: 4,
                    border: `1px solid ${level === l ? '#60a5fa' : '#3a3a4e'}`,
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 8, color: '#4a4a62', letterSpacing: 2, marginBottom: 6 }}>PARENT (optional)</div>
              <select
                value={parent}
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <button
            ...
            style={{
              ...
              background: title.trim() ? '#60a5fa' : '#24243a',
              color: title.trim() ? '#12121c' : '#4a4a62',
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: 'rgba(22, 22, 36, 0.6)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderRadius: 8, border: '1px solid #c084fc30',
      }}>
        ...
        {!result ? (
          <>
            {/* Characters */}
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 8, color: '#4a4a62', letterSpacing: 2, marginBottom: 6 }}>CHARACTERS</div>
              <div style={{
                maxHeight: 120, overflow: 'auto', padding: 8, background: '#16161f',
                borderRadius: 4, border: '1px solid #3a3a4e',
              }}>
                {characters.length > 0 ? characters.map(c => (
                  ...
                  <div style={{ fontSize: 9, color: '#4a4a62', fontStyle: 'italic' }}>
            ...
            {/* Location */}
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 8, color: '#4a4a62', letterSpacing: 2, marginBottom: 6 }}>LOCATION</div>
              <select
                value={location}
                onChange={e => setLocation(e.target.value)}
                style={{
                  width: '100%', padding: '6px 10px', background: '#16161f',
                  border: '1px solid #3a3a4e', borderRadius: 4, color: '#c8cad0',
                  fontFamily: 'inherit', fontSize: 10, outline: 'none',
                }}
              >
                <option value="">Select location...</option>
            ...
            {/* Goal */}
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 8, color: '#4a4a62', letterSpacing: 2, marginBottom: 6 }}>SCENE GOAL</div>
              <textarea
                value={goal}
            ...
            {/* Style */}
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 8, color: '#4a4a62', letterSpacing: 2, marginBottom: 6 }}>FORMAT</div>
              <div style={{ display: 'flex', gap: 4 }}>
                {['fountain', 'prose'].map(s => (
                  <button key={s} onClick={() => setStyle(s)} style={{
                    padding: '4px 10px', fontSize: 9, borderRadius: 4,
                    border: `1px solid ${style === s ? '#c084fc' : '#3a3a4e'}`,
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                style={{
                  padding: '6px 14px', borderRadius: 4, border: 'none',
                  background: (selectedChars.length && location && goal.trim()) ? '#c084fc' : '#24243a',
                  color: (selectedChars.length && location && goal.trim()) ? '#12121c' : '#4a4a62',
                  fontWeight: 700, fontSize: 10,
          ...
          <>
            {/* Generated result */}
            <div style={{
              marginBottom: 12, padding: 12, background: '#16161f', borderRadius: 6,
            ...
            {result.meta && (
              <div style={{ fontSize: 8, color: '#4a4a62', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button onClick={() => {
                ...
                padding: '6px 14px', borderRadius: 4, border: 'none',
                background: '#4ade80', color: '#12121c', fontWeight: 700,
...
export default function KozmoScribo({ savedState, onSaveState, onSetDirty, onNavigateToEntity }) {
  ...
  const { messages: chatMessages, isStreaming: chatStreaming, send: chatSend, stop: chatStop, setPageContext } = useKozmoChat();
  ...
  const [overlayMode, setOverlayMode] = useState('off'); // 'off' | 'pins' | 'full'
  ...
  // If navigating from LAB with a target scene, expand ancestors + auto-select
  useEffect(() => {
    if (tree) {
      const ids = new Set([tree.id]);
      (tree.children || []).forEach(c => ids.add(c.id));

      // If savedState has expandedNodes (e.g. from LAB navigation), merge them in
      if (savedState?.expandedNodes?.length) {
        savedState.expandedNodes.forEach(id => ids.add(id));
      }

      // If navigating from LAB, also expand all ancestors of the target scene
      if (savedState?._navigatedFromLab && savedState?.selectedScene) {
        const path = buildPath(tree, savedState.selectedScene);
        if (path) {
      ...
      // If navigating from LAB with a target scene, select it and fetch its document
      if (savedState?._navigatedFromLab && savedState?.selectedScene) {
        const targetId = savedState.selectedScene;
        setSelected(targetId);
        const node = findNode(tree, targetId);
        if (node && (!node.children || node.children.length === 0)) {
          fetchDocument(targetId);
        }
      }
    }
  }, [tree?.id]);
  ...
  // Chat is now handled by useKozmoChat (streaming via /persona/stream)
  ...
  // Feed page context to Luna so she knows what's on screen
  useEffect(() => {
    const parts = [];
    if (activeProject?.name) parts.push(`Project: ${activeProject.name}`);
    if (currentNode?.title) parts.push(`Scene: ${currentNode.title}`);
    if (sceneCharacters.length) parts.push(`Characters present: ${sceneCharacters.join(', ')}`);
    if (sceneWordCount) parts.push(`Word count: ${sceneWordCount}`);
    if (breadcrumb.length > 1) parts.push(`Path: ${breadcrumb.map(b => b.title || b.id).join(' > ')}`);
    if (currentDocument?.body) parts.push(`Scene text (first 500 chars): ${currentDocument.body.slice(0, 500)}`);
    setPageContext(parts.length ? parts.join('\n') : '');
  }, [activeProject?.name, currentNode?.title, sceneCharacters, sceneWordCount, breadcrumb, currentDocument?.body, setPageContext]);

  // Build paragraph data for overlay (split body into paragraphs with entity positions)
  const overlayParagraphs = useMemo(() => {
    if (!currentDocument?.body) return [];
    return currentDocument.body.split('\n\n').filter(Boolean).map((text, i) => {
      // Detect paragraph type
      const trimmed = text.trim();
      let type = 'prose';
      if (/^#\s/.test(trimmed) || (i === 0 && trimmed.length < 60 && !trimmed.includes('.'))) type = 'title';
      else if (/^(PART|CHAPTER|ACT|SECTION)\b/i.test(trimmed)) type = 'section';

      // Find entity positions in this paragraph text
      const paraEntities = [];
      if (entitiesBrief && entitiesBrief.length > 0) {
        entitiesBrief.forEach(e => {
          if (!e.name) return;
      ...
      return {
        ...
        text: trimmed,
        type,
        entities: paraEntities.sort((a, b) => a.start - b.start),
      };
    });
  }, [currentDocument?.body, entitiesBrief]);

  // Build scene entities for overlay tag bar
  const overlaySceneEntities = useMemo(() => {
    return sceneCharacters
      .map(charId => {
        const e = entities[charId];
        if (!e) return null;
        return { name: e.name, color: entityColor(e), slug: charId, type: e.type };
      })
      .filter(Boolean);
  }, [sceneCharacters, entities]);

  // Build scene metadata for overlay header
  const overlaySceneMeta = useMemo(() => {
    const fm = currentDocument?.frontmatter || {};
    return {
      title: currentDocument?.title || '',
      status: fm.status || currentDocument?.status || '',
      location: fm.location || '',
      time: fm.time || '',
    };
  }, [currentDocument]);
  ...
  return (
    <div style={{
      ...
      background: '#16161f', color: '#e2e8f0',
      ...
      <div style={{
        ...
        borderBottom: '1px solid #282840', padding: '0 12px',
        background: 'rgba(20, 20, 30, 0.4)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)',
        gap: 12, flexShrink: 0,
      }}>
        <div style={{
        ...
        {currentDocument && (
          <div style={{
            ...
            <span style={{
              color: '#5a5a6e', fontSize: 9, padding: '0 6px',
              fontFamily: "'JetBrains Mono', monospace",
            }}>OVERLAY</span>
            {[
              ...
              <button
                ...
                style={{
                  ...
                  color: overlayMode === mode.id ? '#c084fc' : '#5a5a6e',
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        {!leftCollapsed && (
          <div style={{
            width: 260, borderRight: '1px solid #282840', display: 'flex', flexDirection: 'column',
            background: 'rgba(20, 20, 30, 0.4)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', flexShrink: 0,
          }}>
            <div style={{
              padding: '10px 12px', borderBottom: '1px solid #282840',
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            }}>
              <span style={{
                fontSize: 10, color: '#5a5a6e', fontFamily: "'JetBrains Mono', monospace",
              ...
              <span
                ...
                style={{ color: '#3a3a4e', fontSize: 12, cursor: 'pointer' }}
              >
                ◂
              </span>
            </div>
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '4px 0' }}>
              {displayTree ? (
                ...
                <div style={{ padding: 16, color: '#3a3a4e', fontSize: 12, textAlign: 'center' }}>
            ...
            {/* Create buttons */}
            <div style={{
              borderTop: '1px solid #282840', padding: '6px 12px',
              display: 'flex', alignItems: 'center', gap: 6,
            }}>
              ...
              <button onClick={() => setShowCreateForm('container')} style={{
                padding: '3px 8px', fontSize: 8, borderRadius: 3,
                border: '1px dashed #5a5a6e40', background: 'transparent',
                color: '#5a5a6e', cursor: 'pointer', fontFamily: 'inherit',
              }}>+ Container</button>
            ...
            {/* Quick stats */}
            <div style={{
              padding: '10px 12px', borderTop: '1px solid #282840',
              ...
              {stats.map(s => (
                <div key={s.label} style={{
                  ...
                  <div style={{ color: '#3a3a4e', fontSize: 9, fontFamily: "'JetBrains Mono', monospace" }}>
                    {s.label}
                  </div>
        ...
        {leftCollapsed && (
          <div
            ...
            style={{
              width: 24, borderRight: '1px solid #282840',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              cursor: 'pointer', color: '#3a3a4e', fontSize: 12,
        ...
        {/* Center: Editor or Overlay */}
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          {overlayMode !== 'off' && currentDocument ? (
            <ScriboOverlay
              docSlug={selected}
              paragraphs={overlayParagraphs}
              sceneEntities={overlaySceneEntities}
              sceneMeta={overlaySceneMeta}
            ...
            <>
              <SceneEditor content={currentDocument} entities={entitiesBrief} annotatedEntities={kozmoAnnotatedEntities} onNavigateToEntity={onNavigateToEntity} />
        ...
        {rightCollapsed && (
          <div
            ...
            style={{
              width: 24, borderLeft: '1px solid #282840',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              cursor: 'pointer', color: '#3a3a4e', fontSize: 12,
              background: 'rgba(20, 20, 30, 0.4)',
            }}
        ...
        {!rightCollapsed && (
          <div style={{
            width: 300, borderLeft: '1px solid #282840', display: 'flex', flexDirection: 'column',
            background: 'rgba(20, 20, 30, 0.4)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', flexShrink: 0,
          }}>
            <div style={{
              padding: '10px 12px', borderBottom: '1px solid #282840',
              ...
              {[
                ...
                <button key={tab.id} onClick={() => setRightPanel(tab.id)} style={{
                  ...
                  color: rightPanel === tab.id ? '#c084fc' : '#5a5a6e',
                  fontSize: 10, cursor: 'pointer', fontFamily: "'JetBrains Mono', monospace",
              ...
              <span
                ...
                style={{ color: '#3a3a4e', fontSize: 12, cursor: 'pointer', marginLeft: 4 }}
              >
                ▸
              </span>
            </div>
            <div style={{ flex: 1, overflow: 'hidden' }}>
              {rightPanel === 'chat' ? (
                <KozmoChat
                  ...
                  onStop={chatStop}
                  agents={AGENTS}
                  placeholder={currentDocument ? `Ask Luna about ${currentNode?.title || 'this scene'}...` : 'Select a scene...'}
                />
              ) : rightPanel === 'tracker' ? (
                <KeywordTracker
                  reverseIndex={reverseIndex}
                  onNavigateToScene={(sceneSlug) => handleSelect(sceneSlug, null)}
                  onNavigateToEntity={(slug, type) => onNavigateToEntity?.(slug, type)}
                />
              ) : (
