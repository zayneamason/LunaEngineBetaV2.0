  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        />
        <span style={{ fontSize: 'var(--ec-fs-xs)', fontWeight: 600, color: 'var(--ec-text-primary)' }}>
          {channel.name || channel.id}
        </span>
      </div>
      <div style={{ fontSize: 'var(--ec-fs-label)', color: 'var(--ec-text-faint)', textTransform: 'uppercase' }}>
        {channel.state}
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--ec-fs-label)', color: 'var(--ec-text-secondary)' }}>
...
function ArtifactRow({ a }) {
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--ec-fs-label)', color: 'var(--ec-text-faint)', marginBottom: 2 }}>
        <span style={{ color, fontWeight: 600 }}>
          [{channelId}] {a.node_type}
        </span>
        <span>{fmtTime(a.created_at)}</span>
      </div>
      <div style={{ fontSize: 'var(--ec-fs-xs)', color: 'var(--ec-text-secondary)', lineHeight: 1.3 }}>
        {a.content}
      </div>
    </div>
  );
...
function TraitBar({ name, value, nudge }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 'var(--ec-fs-label)' }}>
...
export default function RadioWidget() {
  ...
  if (error && !status) {
    return (
      <div style={{ textAlign: 'center', padding: 24, color: 'var(--ec-text-faint)', fontSize: 'var(--ec-fs-sm)' }}>
        LunaFM: {error}
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ textAlign: 'center', padding: '4px 0' }}>
        <div className="ec-font-display" style={{ fontSize: 'var(--ec-fs-lg)', fontWeight: 300, color: 'var(--ec-accent-voice)' }}>
          ◉ LunaFM
        </div>
        <div style={{ fontSize: 'var(--ec-fs-label)', color: 'var(--ec-text-faint)', marginTop: 2 }}>
      ...
      {pollution?.buckets && (
        <div className="ec-glass-interactive" style={{ padding: '8px 10px', borderRadius: 6 }}>
          <div className="ec-font-label" style={{ fontSize: 'var(--ec-fs-label)', color: 'var(--ec-text-faint)', marginBottom: 6 }}>
            POLLUTION
          </div>
          {['1h', '24h', 'total'].map((bucket) => {
            ...
            return (
              <div key={bucket} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 'var(--ec-fs-label)', color: 'var(--ec-text-secondary)', marginBottom: 2 }}>
                <span style={{ width: 32, color: 'var(--ec-text-faint)' }}>{bucket}</span>
                <span style={{ width: 28, textAlign: 'right' }}>{total}</span>
                <span style={{ flex: 1, color: 'var(--ec-text-faint)', fontSize: 'var(--ec-fs-label)' }}>
      ...
      <div className="ec-glass-interactive" style={{ padding: '10px 12px', borderRadius: 6 }}>
        <div className="ec-font-label" style={{ fontSize: 'var(--ec-fs-label)', color: 'var(--ec-text-faint)', marginBottom: 8 }}>
          THOUGHT STREAM
        </div>
        {artifacts.length === 0 ? (
          <div style={{ fontSize: 'var(--ec-fs-xs)', color: 'var(--ec-text-faint)', textAlign: 'center', padding: 8 }}>
            No artifacts yet
          </div>
        ) : (
      ...
      <div className="ec-glass-interactive" style={{ padding: '10px 12px', borderRadius: 6 }}>
        ...
        <div style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 'var(--ec-fs-label)', color: 'var(--ec-text-faint)', marginBottom: 4 }}>
        ...
        {/* Trait bars */}
        <div style={{ fontSize: 'var(--ec-fs-label)', color: 'var(--ec-text-faint)', marginBottom: 4 }}>COGNITIVE STATE</div>
        {hasTraits ? (
          ...
          <div style={{ fontSize: 'var(--ec-fs-label)', color: 'var(--ec-text-faint)', textAlign: 'center', padding: 6 }}>
