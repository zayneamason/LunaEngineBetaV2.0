...
**Implementation Accuracy:** 85% (see Audit notes)

> **Audit Notes (January 25, 2026):**
> - **Actor count corrected:** 5 actors implemented, not 6 (Voice and Oven actors NOT implemented)
> - **HistoryManager actor added:** Three-tier conversation history (not in original Bible)
> - **on_idle() hook NOT implemented:** Actor loop has no idle callback in actual code
> - **Lifecycle hooks updated:** Actual implementation uses `handle()` not `handle_message()`
...
### Engine State Machine (Actual v2.0)
...
```
  ...
  pause()    sleep()    stop()
     ...
     ▼          ▼          ▼
┌────────┐ ┌────────┐ ┌────────┐
│ PAUSED │ │SLEEPING│ │ STOPPED│ ← Terminal state
└────────┘ └────────┘ └────────┘
     │          │
  resume()   wake()
     │          │
