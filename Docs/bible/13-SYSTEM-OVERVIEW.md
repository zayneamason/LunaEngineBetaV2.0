# LUNA ENGINE — System Overview (v3.0)
...
> **v3.0 Audit Notes:**
> - Updated to reflect actual implementation (5 actors, sqlite-vec vectors)
> - Oven Actor NOT implemented — delegation handled in Director
> - FAISS references updated to sqlite-vec
...
│   └─────────────────────────────────────────────────────┘   │
│                                                             │
│   sqlite-vec extension    (vector search in memory_matrix)  │
...
| sqlite-vec | Conceptual/semantic queries | ~30ms |
...
│                                                             │
│   ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  │
│   │ DIRECTOR │  │  MATRIX  │  │  SCRIBE  │  │ LIBRARIAN│  │ HISTORY  │  │
│   │  Actor   │  │  Actor   │  │  (Ben)   │  │  (Dude)  │  │ MANAGER  │  │
│   │          │  │          │  │          │  │          │  │          │  │
...
**Actor Responsibilities (5 actors implemented):**

| Actor | Alias | Job | Status |
|-------|-------|-----|--------|
| Director | — | LLM inference, Claude delegation, routing | Implemented |
| Matrix | — | Memory substrate (sqlite-vec + NetworkX) | Implemented |
| Scribe | Ben Franklin | Extract memories from conversations | Implemented |
| Librarian | The Dude | File memories, entity resolution, edges | Implemented |
| HistoryManager | — | Three-tier conversation history | Implemented |
| ~~Oven~~ | — | ~~Execute async tasks~~ | NOT IMPL (handled in Director) |
...
> **v3.0 Note:** Oven Actor NOT implemented. Delegation is handled directly in DirectorActor via `_should_delegate()` and `_generate_with_delegation()`.

```
┌─────────────────────────────────────────────────────────────┐
│                   SHADOW REASONER                           │
│                                                             │
│   User: "Research the latest sqlite-vec optimizations"     │
│                                                             │
│   ┌─────────────────────────────────────────────────────┐  │
│   │  Director generates:                                 │  │
│   │                                                      │  │
│   │  "I'll look into that. <REQ_CLAUDE>                 │  │
│   │   Research latest sqlite-vec optimization techniques│  │
...
│                          ▼                                  │
│   ┌─────────────────────────────────────────────────────┐  │
│   │  Director Actor (handles delegation internally):     │  │
│   │  • Detects <REQ_CLAUDE> token                       │  │
│   │  • Calls Claude API via _should_delegate()          │  │
│   │  • Integrates result into Luna's voice              │  │
│   └─────────────────────────────────────────────────────┘  │
│                                                             │
...
│   │  4. Generate embeddings → sqlite-vec             │     │
│   │                                                   │     │
│   │  DEFERRED (batched):                             │     │
...
│   │  8. Vector index optimization if needed          │     │
│   └──────────────────────────────────────────────────┘     │
│                                                             │
...
│                            ▼                                │
│   ┌─────────────────────────────────────────────────────┐  │
│   │  Director Actor (delegation logic):                  │  │
│   │  • Detects delegation task                          │  │
...
│  Director ──<REQ_CLAUDE>──▶ Claude API ──▶ back to Director            │
│  (delegation handled internally in Director, no separate Oven actor)   │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
## 9. Implementation Status (v3.0)

| Component | Status | Notes |
|-----------|--------|-------|
| **5 Actors** | Implemented | Director, Matrix, Scribe, Librarian, HistoryManager |
| **Oven Actor** | NOT IMPL | Delegation handled in Director |
| **Voice Actor** | NOT IMPL | Voice I/O via PersonaAdapter |
| **FAISS Vectors** | REPLACED | Using sqlite-vec extension |
| **FTS5 Search** | Partial | Defined in schema, primary path uses LIKE |
| **KV Cache** | NOT IMPL | Future optimization |
| **7B Warm Path** | NOT IMPL | Only 3B + Claude delegation |
| **Speculative Retrieval** | NOT IMPL | Future optimization |
...
*This overview distills the full Bible into a single reference document. Updated for v3.0 Bible audit.*
