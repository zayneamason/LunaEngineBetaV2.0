# Bible v3.0: Part VIII - Delegation Protocol
...
> - **No Oven Actor:** Delegation handled directly in DirectorActor via `_should_delegate()` method
> - **REQ_CLAUDE tokens:** Defined as class constants `REQ_CLAUDE_START` and `REQ_CLAUDE_END`
> - **Stats tracking:** Uses `_delegated_generations` counter (not `cloud_generations`)
> - **Shadow Reasoner pattern:** Implemented inline in Director, not as separate actor
...
### Implementation (Actual v3.0)

> **Note:** The Oven Actor described in the original spec is NOT implemented.
> Delegation is handled directly in DirectorActor via `_should_delegate()` method.

```python
# From src/luna/actors/director.py

class DirectorActor(Actor):
    """Handles delegation inline - no separate Oven Actor."""

    # Delegation token constants
    ...
    async def _should_delegate(self, user_message: str) -> bool:
        ...
        Uses heuristics and keyword detection to route complex queries.
        """
        # Implementation checks for:
        # - Research/analysis keywords
        # - Code generation requests
        # - Long-form content requests
        # - Questions beyond local knowledge
        ...

    async def _generate_local_with_delegation(
        self,
        user_message: str,
        ...
    ) -> None:
        ...
        then stream Claude's response in Luna's voice.
        """
        response_buffer = ""
        delegation_detected = False

        async for token in self._local.generate_stream(prompt):
            response_buffer += token

            # Check for delegation signal
            if self.REQ_CLAUDE_START in response_buffer and not delegation_detected:
                delegation_detected = True
                # Extract and stream acknowledgment
                # Then delegate to Claude
                ...

        if delegation_detected:
            self._delegated_generations += 1
            # Call Claude API directly
            # Return facts for narration
```
...
### Adding a New Worker (v3.0)

> **Note:** In the actual implementation, workers are managed via ProviderRegistry
> in `src/luna/llm/registry.py`, not via an Oven actor.
...
from luna.llm.registry import ProviderRegistry

registry = ProviderRegistry()
registry.register("gemini", GeminiProvider())

# 3. Director uses _should_delegate() to route
# Providers are selected based on availability and task type

# 4. Training data for local model delegation (optional)
{
    "instruction": "Analyze the performance characteristics of these three algorithms",
    "response": "<REQ_CLAUDE>This is an analysis task. Let me look into that.</REQ_CLAUDE>"
}
```

> **Implementation Note:** The current `_should_delegate()` method routes to Claude
> as the primary cloud provider. Multi-provider routing is supported via ProviderRegistry.
...
| Component | Role | Implementation Status (v3.0) |
|-----------|------|------------------------------|
| Delegation Boundary | What Luna handles vs. delegates | Implemented in `_should_delegate()` |
| `<REQ_CLAUDE>` Token | Learned delegation signal | Implemented as class constants |
| Shadow Reasoner | Background execution, foreground narration | Inline in Director (no Oven Actor) |
| Filler & Continuity | No awkward silence | Partial (acknowledgment only) |
| Worker Contracts | Standardized request/response | Via ProviderRegistry |
| Privacy Controls | Minimal data sent to cloud | Implemented |

### Key Implementation Notes (v3.0)

- **No Oven Actor:** Delegation handled directly in DirectorActor
- **Stats field:** `_delegated_generations` (not `cloud_generations`)
- **Token detection:** Uses `REQ_CLAUDE_START` and `REQ_CLAUDE_END` constants
- **Provider system:** Multi-provider support via `src/luna/llm/registry.py`
