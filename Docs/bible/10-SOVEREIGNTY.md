...
**Version:** 3.0
**Date:** 2026-01-30
**Status:** CURRENT — Critical security issues identified
**Core Principle:** "Luna is a file. Copy the file, copy Luna."

---

## CRITICAL SECURITY NOTICE (v3.0 Update)

> **3 SECURITY VIOLATIONS IDENTIFIED** — See AUDIT-CONFIG.md

### Active Security Issues

| Issue | Severity | Status | Reference |
|-------|----------|--------|-----------|
| API keys in `.env` committed to repo | CRITICAL | OPEN | BUG-001 |
| Truncated keys in diagnostic output | HIGH | OPEN | BUG-001 |
| No API authentication on 74 endpoints | HIGH | OPEN | INSIGHTS.md |

### Immediate Actions Required

1. **ROTATE ALL API KEYS** — Anthropic, Groq, Google keys are exposed
2. **Add `.env` to `.gitignore`** — Prevent future leaks
3. **Scrub git history** — Use BFG Repo-Cleaner to remove `.env` from history
4. **Delete `DIAGNOSTIC_OUTPUT.txt`** — Contains partial key exposure
5. **Sanitize diagnostic scripts** — Mask API key values in output

### API Key Handling (Current vs Required)

| Aspect | Current State | Required State |
|--------|---------------|----------------|
| Storage | `.env` file (tracked in git) | `.env` file (gitignored) |
| Config reference | `api_key_env` in JSON (correct) | Keep using env var names |
| Diagnostic output | Keys partially visible | Keys fully masked |
| Validation | Partial (checks if non-empty) | Add format validation |

### What's Working

The architecture for API key handling is **correct**:
- `llm_providers.json` stores env var names, not actual keys
- `security.py` has forbidden patterns including `.env`
- Code reads keys from environment at runtime

The **implementation** has a leak: the `.env` file itself is tracked.

---

## 10.1 The Threat Model

> **Status:** Threat model is sound. Implementation has gaps (see above).
...
> **Status:** PARTIAL. The vault concept exists, but current implementation uses `data/luna_engine.db` directly without encryption wrapper. The encrypted sparse bundle is aspirational.
...
> **Status:** NOT IMPLEMENTED. The Dead Man's Switch is aspirational architecture. No heartbeat file, no automatic lockdown.
...
> **Status:** PARTIALLY IMPLEMENTED. Luna's data is in SQLite (portable), but the full vault/migration tooling does not exist.
...
> **Status:** MOSTLY IMPLEMENTED with caveats. No telemetry, local-first, but cloud delegation sends prompts to external APIs (Groq, Claude, Gemini).
...
| Action | Why | v3.0 Status |
|--------|-----|-------------|
| Work offline | Cloud is optional, not required | PARTIAL — Local inference slow (BUG-016) |
| Respect vault boundaries | Nothing escapes the container | NO VAULT — Uses unencrypted `data/` |
| Clean up on shutdown | No traces left | NOT IMPLEMENTED |
| Encrypt at rest | Data protected when dormant | NOT IMPLEMENTED |
...
> **Status:** NOT IMPLEMENTED. No automated backup system exists. SQLite file can be manually copied.
...
> **Status:** PARTIAL. Data is in SQLite (extractable), but no formal export tools exist.
...
|-----------|---------|----------------------|
| Encrypted Vault | All Luna data in one encrypted container | NOT IMPLEMENTED |
| Dead Man's Switch | Automatic lockdown on stale heartbeat | NOT IMPLEMENTED |
| Portability | Copy vault = copy Luna | PARTIAL (SQLite is portable) |
| Data Sovereignty | No telemetry, no extraction, no phoning home | IMPLEMENTED (no telemetry) |
| Backup & Recovery | Multiple tiers, verified restoration | NOT IMPLEMENTED |
| Escape Hatch | Full export to portable formats | PARTIAL (raw SQLite accessible) |
| API Key Security | Secrets never committed | VIOLATED (BUG-001) |

### Critical Action Items

1. **Rotate API keys** — Current keys are compromised
2. **Fix `.gitignore`** — Add `.env` pattern
3. **Scrub git history** — Remove committed secrets
4. **Implement encryption at rest** — Either vault or LUKS/FileVault

### What's Working

- **Single-file database** — Luna's soul is in one SQLite file
- **No telemetry** — No data sent to analytics services
- **Environment variable pattern** — Config references env vars, not keys
- **Local-first design** — Cloud is optional (though currently needed for speed)
