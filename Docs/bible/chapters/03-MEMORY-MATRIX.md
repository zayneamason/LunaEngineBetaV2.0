# Part III: The Memory Matrix (Substrate) — v3.0
...
**Change Log:**
- v3.0: FTS5 confirmed implemented with Porter stemmer; documented Memory Economy; added cluster-level lock-in reference; accuracy pass from Phase 5 audit
- v2.2: Corrected table names (`memory_nodes`, `graph_edges`), documented actual schema, updated sqlite-vec references
...
├── memory_nodes_fts (FTS5 virtual table for full-text search)
└── indexes          (B-tree for filtering)
```

> **Note:** FTS5 full-text search IS implemented with Porter stemmer and unicode61 tokenizer. The `fts5_search()` method uses BM25 ranking. Sync triggers keep FTS5 in sync with `memory_nodes` on INSERT/UPDATE/DELETE.
...
```
    ...
    |             |  |             |  |             |
    |  Full-text  |  |  Semantic   |  |  Traversal  |
    |  BM25 rank  |  |   vectors   |  |   (BFS)     |
    |    ~10ms    |  |   ~10ms     |  |    ~5ms     |
    +------+------+  +------+------+  +------+------+
           |                |                |
           +--------+-------+--------+-------+
...
> **Implementation Note:** FTS5 is fully implemented with Porter stemmer and unicode61 tokenizer. The `fts5_search()` method uses BM25 ranking. A `search_nodes()` method also exists using LIKE patterns as a fallback. The `hybrid_search()` method combines FTS5 + semantic search using Reciprocal Rank Fusion (RRF).
...
| FTS5 | Full-text search with Porter stemmer, BM25 ranking |
...
| Memory Economy | Cluster-level organization (see below) |

### Memory Economy (v3.0)

The Memory Economy provides cluster-level organization on top of individual memory nodes:

| Component | Purpose | Implementation |
|-----------|---------|----------------|
| `ClusterManager` | CRUD for semantic clusters | `memory/cluster_manager.py` |
| `ClusteringEngine` | Keyword-based grouping (Jaccard similarity) | `memory/clustering_engine.py` |
| `LockInCalculator` | Cluster-level lock-in with decay | `memory/lock_in.py` |
| `ConstellationAssembler` | Token-budgeted context assembly | `memory/constellation.py` |

**Cluster-Level Lock-In States:**
| State | Threshold | Decay Lambda | Half-Life |
|-------|-----------|--------------|-----------|
...
> **Note:** Cluster-level thresholds differ from node-level thresholds. See Part III-A for node-level lock-in (0.30/0.70 boundaries).

Configuration: `config/memory_economy_config.json`

### Known Limitations (v3.0)

| Limitation | Impact | Status |
|------------|--------|--------|
| Tag siblings not implemented | 10% of node lock-in weight missing | ❌ NOT IMPLEMENTED |
| Network effects hardcoded to 0 | 20% of node lock-in weight uses estimate | Partial (uses neighbor count / 3) |
| History compression queues exist but not activated | Manual archival only | Planned |
| Memory Economy tables not in schema.sql | Auto-created by ClusterManager | Schema fragmentation |
