...
**Version:** 3.0
**Date:** 2026-01-30
**Status:** CURRENT — Honest assessment of targets vs reality
...
> **IMPORTANT:** This chapter describes performance *targets* and *architecture*. Current implementation does not meet these targets.

### Current vs Target Performance

| Operation | Current (Actual) | Target | Gap | Status |
|-----------|------------------|--------|-----|--------|
| Local inference | **2-3 tok/s** | 50+ tok/s | CRITICAL | BUG-016 |
| First response latency | **>5 seconds** | <500ms | CRITICAL | Blocked by inference |
| Memory search | ~10ms | ~10ms | OK | Working |
| Context assembly | ~50ms | ~50ms | OK | Working |

### Why Local Inference Is Slow (BUG-016)

The local inference bottleneck (2-3 tok/s instead of 50+ tok/s) has several possible causes:

1. **Model may not be 4-bit quantized** — Running float16 instead of int4
2. **MLX may be using CPU** — Check `mx.default_device()` returns GPU
3. **Context window overhead** — Large context slows generation
4. **Model size** — 7B may be too large for available unified memory

**Current Workaround:** System defaults to cloud delegation (Groq/Claude) for most responses.

### Speculative Execution — NOT YET IMPLEMENTED

The speculative retrieval and KV cache warming described in this chapter are **aspirational architecture**, not current implementation. The codebase has TODO comments for these features.

---

---

## 9.1 The Latency Budget (Target Architecture)
...
## 9.2 Speculative Execution (NOT YET IMPLEMENTED)

> **Status:** Aspirational architecture. No implementation exists.
...
## 9.3 KV Cache Management (NOT YET IMPLEMENTED)

> **Status:** Aspirational architecture. KV cache pinning is not implemented. The `identity_buffer.safetensors` file mentioned does not exist.
...
> **Status:** PARTIAL. sqlite-vec is implemented for vector search, but FAISS is not used. The codebase uses 384-dimension local embeddings.

### The Goldilocks Zone

At Luna's scale (~60K nodes as of 2026-01-30), index selection matters.
...
> **Status:** NOT IMPLEMENTED. The codebase uses NetworkX. rustworkx migration has not occurred.

### The Problem

NetworkX is pure Python. Graph operations that should take microseconds take milliseconds.

### The Solution: rustworkx (PROPOSED)
...
## 9.6 Bloom Filter Pre-check (NOT YET IMPLEMENTED)

> **Status:** Aspirational architecture. No Bloom filter implementation exists.
...
## 9.7 Query Routing (PARTIAL)

> **Status:** Basic routing exists in the codebase, but the sophisticated heuristic router and TinyModel router are not implemented.
...
> **Status:** IMPLEMENTED. RRF merge is available in the hybrid search path.
...
## 9.9 Parallel Prefetching (NOT YET IMPLEMENTED)

> **Status:** Aspirational architecture. No prefetching implementation exists.
...
## 9.10 The Complete Optimized Pipeline (TARGET ARCHITECTURE)

> **Status:** This is the target architecture. Current implementation uses a simpler pipeline with cloud delegation as the primary path.

### Flow Diagram (Aspirational)
...
| Optimization | Target Savings | Implementation Status |
|--------------|----------------|----------------------|
| Speculative execution | ~50ms | NOT IMPLEMENTED |
| KV cache pinning | ~50-100ms | NOT IMPLEMENTED |
| rustworkx | ~45ms | NOT IMPLEMENTED (using NetworkX) |
| Query routing | ~20-30ms | PARTIAL |
| Bloom filter | ~5-10ms | NOT IMPLEMENTED |
| RRF merge | Better results | IMPLEMENTED |
| Parallel prefetch | Amortized | NOT IMPLEMENTED |

### Current Reality

The current system does not achieve <500ms latency due to:

1. **Local inference bottleneck** (BUG-016): 2-3 tok/s instead of 50+
2. **No speculative execution**: All retrieval happens after speech ends
3. **No KV cache management**: Identity buffer re-encoded every turn
4. **NetworkX graph traversal**: Pure Python, not optimized

### Path Forward

1. Fix local inference performance (verify quantization, GPU usage)
2. Implement KV cache pinning for identity buffer
3. Add speculative retrieval on partial transcripts
4. Consider rustworkx for graph operations at scale
