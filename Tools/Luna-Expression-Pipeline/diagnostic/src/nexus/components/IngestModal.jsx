...
const PATTERNS = [
  { value: 'utilitarian', label: 'UTILITARIAN', color: '#60a5fa',
    desc: 'Standard decay. Search-driven lock-in.' },
  { value: 'ceremonial',  label: 'CEREMONIAL',  color: '#f97316',
    desc: 'Sovereign floor 0.30. Never decays.' },
  { value: 'emergent',    label: 'EMERGENT',    color: '#34d399',
    desc: 'Luna-built. Starts high, decays slowly.' },
];
...
export default function IngestModal({ collection, onClose, onIngest }) {
  ...
  const [pattern, setPattern] = useState(collection.ingestion_pattern || 'utilitarian');
      ...
      <div style={{
        ...
        <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <span style={{ fontSize: 7, fontFamily: LABEL, letterSpacing: 2, color: 'var(--ec-text-muted)' }}>INGESTION PATTERN</span>
            <div style={{ display: 'flex', gap: 4 }}>
              {PATTERNS.map(p => (
                <div key={p.value} onClick={() => setPattern(p.value)} style={{
                  flex: 1, padding: '6px 4px', borderRadius: 5, cursor: 'pointer',
                  textAlign: 'center',
                  background: pattern === p.value ? `${p.color}15` : 'transparent',
                  border: `1px solid ${pattern === p.value ? `${p.color}50` : 'var(--ec-border)'}`,
                  transition: 'all 0.15s',
                }}>
                  <div style={{ fontSize: 8, fontFamily: LABEL, letterSpacing: 1,
                    color: pattern === p.value ? p.color : 'var(--ec-text-muted)',
                    marginBottom: 2 }}>{p.label}</div>
                  <div style={{ fontSize: 7, color: 'var(--ec-text-faint)',
                    fontFamily: MONO, lineHeight: 1.3 }}>{p.desc}</div>
                </div>
              ))}
            </div>
          </div>

          <div onClick={() => { if (path) { onIngest(collection.key, mode, path, pattern); onClose(); }}} style={{
