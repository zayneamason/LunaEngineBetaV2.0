...
export function useNexus() {
  ...
  const fetchStats = useCallback(async (key) => {
    try {
      const res = await fetch(`${API_BASE}/api/nexus/${encodeURIComponent(key)}/stats`);
  ...
  const fetchDocuments = useCallback(async (key, skip = 0, limit = 50) => {
    try {
      const res = await fetch(`${API_BASE}/api/nexus/${encodeURIComponent(key)}/documents?skip=${skip}&limit=${limit}`);
      return await res.json();
    } catch (e) {
      setError(e.message);
      return { documents: [] };
  ...
  const search = useCallback(async (collection, query, type = 'semantic') => {
    try {
      const res = await fetch(`${API_BASE}/api/nexus/search`, {
        method: 'POST',
  ...
  const ingest = useCallback(async (collection, filePath, metadata) => {
    setIsLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/nexus/ingest`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const activateProject = useCallback(async (slug) => {
    try {
      await fetch(`${API_BASE}/project/activate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const deactivateProject = useCallback(async () => {
    try {
      await fetch(`${API_BASE}/project/deactivate`, { method: 'POST' });
      await fetchProjects();
    } catch (e) { setError(e.message); }
  }, [fetchProjects]);
